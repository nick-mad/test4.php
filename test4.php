...
function row_cells($row_cells, $columns_headers, $columns_lengths)
{
    $row = '';
    foreach ($columns_headers as $header) {
        $r = $header == 'Color' ? '<span style="color:' . $row_cells[$header] .'">' : '<span>'; 
        $row .= LINE_Y_CHAR . str_repeat(' ', SPACING_X) . $r . str_pad($row_cells[$header], SPACING_X + $columns_lengths[$header], ' ', STR_PAD_RIGHT) .'</span>';
    }
    $row .= LINE_Y_CHAR;
